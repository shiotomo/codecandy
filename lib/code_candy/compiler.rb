module CodeCandy
  class Compiler
    ...
    def initialize(container, data)
      ...
      # プログラムのファイルの作成
      File.open("/tmp/#{@work_dir}/#{@language_data[:source_file]}", "w") do |f|
    ...
    def exec
      begin
        Timeout.timeout(@language_data[:time_out]) do
          # === 実行ここから ===
          @container.start
          # container_cmd = "cd /workspace && /usr/bin/time -q -f \"%e\" -o /workspace/time.txt timeout #{@cmd[:time_out]} #{@cmd[:exec_cmd]} < #{@input_file}"
          container_cmd = "/usr/bin/time -q -f \"%e\" -o /workspace/time.txt timeout #{@language_data[:time_out]} #{@language_data[:exec_cmd]} < #{@input_file}"
